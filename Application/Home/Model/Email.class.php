...
class Email // 大部分云服务器25端口不能使用，搭建云服务器文章：https://www.ywbj.cc/?p=929
{
    /**
    ...
    // 设置smtp邮件服务器，账号、密码
    public function __construct($relay_host = "smtp.exmail.qq.com", $smtp_port = 465, $auth = false, $user = "", $pass = "")
    {
//$mail->setServer("smtp@126.com", "XXXXX@126.com", "XXXXX"); //设置smtp服务器，普通连接方式
        $this->setServer($relay_host, $user, $pass, $smtp_port, true); //设置smtp服务器，到服务器的SSL连接
    }
...
//        }
    ...
    /* Main Function */
    public function send($to, $from, $subject = "", $body = "", $mailtype, $cc = "", $bcc = "", $additional_headers = "", $header = "")
    {
        $mail_from = $this->get_address($this->strip_comment($from));
        $this->setFrom($mail_from); //设置发件人
        $TO = explode(",", $this->strip_comment($to));
        foreach ($TO as $receiver) {
            if (!empty($receiver))
                $this->setReceiver($receiver);
        }
        $cc_list = explode(",", $this->strip_comment($cc));
        foreach ($cc_list as $cc_item) {
            if (!empty($cc_item))
                $this->setCc($cc_item);
        }
        $bcc_list = explode(",", $this->strip_comment($bcc));
        foreach ($bcc_list as $bcc_item) {
            if (!empty($bcc_item))
                $this->setBcc($bcc_item); //设置秘密抄送，多个秘密抄送，调用多次
        }
        //$this->addAttachment("XXXX"); //添加附件，多个附件，调用多次
        $this->setMail($subject, $body); //设置邮件主题、内容
        $res = $this->doSend();
        jdd($this->_errorMessage);
        return $res;
    }
}